...
export default plugin.withOptions((options = { logs: false, useLocalFonts: false }) => ({ addBase }) => {
    let isLog = true;
    if (!options.logs) {
        isLog = false;
    }
    ...
    if (options.useLocalFonts) {
        ...
        if (isLog) {
            ...
            console.log(`├─── Ⓘ You need to copy the fonts to the folder where your not compiled main.css file is located`);
            console.log(`├─── Ⓘ Call npx ${b24UiInfo.name} copy-fonts --dest=assets/css/fonts `);
        }
    }
    else {
        const allowed = [
        ...
        if (isLog) {
            ...
            console.log(`├───── Ⓘ To connect the remaining fonts, set the parameter  useLocalFonts = true in the plugin configuration.`);
        }
    }
    if (isLog) {
