...
TMP=/tmp/bot_fix_resp.txt
rm -f $TMP
# capture response
docker exec -i teachbaseai-backend-1 curl -sS -i -H "Authorization: Bearer ${TOKEN}" -X POST http://127.0.0.1:8000/v1/admin/portals/2/bot/fix-handlers > $TMP
python - <<'PY'
import re,sys
s=open('/tmp/bot_fix_resp.txt','r',encoding='utf-8',errors='replace').read()
